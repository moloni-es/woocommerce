...
class Logs
{
    ...
    public static function removeOlderLogs()
    {
        ...
        $query = $wpdb->prepare(
            "DELETE FROM `" . $wpdb->get_blog_prefix() . "moloni_es_logs` WHERE created_at < %s",
    ...
    private static function getAll(): array
    {
        ...
        $companyId = Storage::$MOLONI_ES_COMPANY_ID ?? 0;

        $query = $wpdb->prepare(
            "SELECT COUNT(*) FROM `" . $wpdb->get_blog_prefix() . "moloni_es_logs` WHERE company_id = %d",
            $companyId
        );
        ...
        $query = $wpdb->prepare(
            "SELECT * FROM `" . $wpdb->get_blog_prefix() . "moloni_es_logs` 
            WHERE
               company_id = %d 
            ORDER BY id DESC
            LIMIT %d
