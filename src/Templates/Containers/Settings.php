                ...
                <td>
                    <select id="document_set_id" name='opt[document_set_id]' class='inputOut'>
                        ...
                        <?php foreach ($documentSets as $documentSet) : ?>
                            <option value='<?= $documentSet['documentSetId'] ?>' <?= (int)DOCUMENT_SET_ID === $documentSet['documentSetId'] ? 'selected' : '' ?>><?= $documentSet['name'] ?></option>
                        <?php endforeach; ?>
                    </select>
                    <p class='description'><?= __('Required', 'moloni_es') ?></p>
                </td>
                    ...
                    <td>
                        <select id="moloni_product_warehouse" name='opt[moloni_product_warehouse]' class='inputOut'>
                            ...
                            <?php foreach ($warehouses as $warehouse) : ?>
                                <option value='<?= $warehouse['warehouseId'] ?>' <?= (int)MOLONI_PRODUCT_WAREHOUSE === $warehouse['warehouseId'] ? 'selected' : '' ?>>
                    ...
                    <select id="measure_unit_id" name='opt[measure_unit]' class='inputOut'>
                        ...
                        <?php if (is_array($measurementUnits)): ?>
                            <?php foreach ($measurementUnits as $measurementUnit) : ?>
                                <option value='<?= $measurementUnit['measurementUnitId'] ?>' <?= (int)MEASURE_UNIT === $measurementUnit['measurementUnitId'] ? 'selected' : '' ?>><?= $measurementUnit['name'] ?></option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
                    <p class='description'><?= __('Required' , 'moloni_es') ?></p>
                    ...
                    <select id="tax_id" name='opt[tax_id]' class='inputOut'>
                        ...
                        <?php if (is_array($taxes)): ?>
                            <?php foreach ($taxes as $tax) : ?>
                                <option value='<?= $tax['taxId'] ?>' <?= (int)TAX_ID === $tax['taxId'] ? 'selected' : '' ?>><?= $tax['name'] . ' ('.$tax['value'].'%)'?></option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
            ...
            <tr>
                ...
                <td>
                    <select id="tax_id_shipping" name='opt[tax_id_shipping]' class='inputOut'>
                        ...
                        <?php if (is_array($taxes)): ?>
                            <?php foreach ($taxes as $tax) : ?>
                                <option value='<?= $tax['taxId'] ?>' <?= (int)TAX_ID_SHIPPING === $tax['taxId'] ? 'selected' : '' ?>><?= $tax['name'] . ' ('.$tax['value'].'%)'?></option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
                    ...
                    <select id="maturity_date_id" name='opt[maturity_date]' class='inputOut'>
                        ...
                        <?php if (is_array($maturityDates)): ?>
                            <?php foreach ($maturityDates as $maturityDate) : ?>
                                <option value='<?= $maturityDate['maturityDateId'] ?>' <?= (int)MATURITY_DATE === $maturityDate['maturityDateId'] ? 'selected' : '' ?>><?= $maturityDate['name'] ?></option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
                    ...
                    <select id="payment_method_id" name='opt[payment_method]' class='inputOut'>
                        ...
                        <?php if (is_array($paymentMethods)): ?>
                            <?php foreach ($paymentMethods as $paymentMethod) : ?>
                                <option value='<?= $paymentMethod['paymentMethodId'] ?>' <?= (int)PAYMENT_METHOD === $paymentMethod['paymentMethodId'] ? 'selected' : '' ?>><?= $paymentMethod['name'] ?></option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
